...
public class ItemController extends BaseController {
    ...
    @GetMapping("/merchant")
    public ModelAndView merchant(ModelAndView mov) {
        List<ItemServiceModel> allItems = this.itemService.getAllItems();
        mov.addObject("items", allItems);
        return super.view("item/merchant", mov);
    }

    @PostMapping("/merchant/{itemName}")
    public ModelAndView merchantPost(@PathVariable String itemName) {
